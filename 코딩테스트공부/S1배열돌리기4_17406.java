...
public class S1배열돌리기4_17406 {
    ...
    public static void rotate(int sn, int sm, int en, int em, int tempmap[][]) {
        ...
        for(int i = en; i > sn; i--) {
            if(i == sn + 1) tempmap[i][em] = temp[0];
            else tempmap[i][em] = tempmap[i - 1][em];
        }
        for(int i = sm; i < em; i++) {
            if(i == em - 1) tempmap[en][i] = temp[1];
            else tempmap[en][i] = tempmap[en][i + 1];
        }
        for(int i = sn; i < en; i++) {
            if(i == en - 1) tempmap[i][sm] = temp[2];
            else tempmap[i][sm] = tempmap[i + 1][sm];
        }    
